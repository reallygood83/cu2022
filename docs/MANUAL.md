...
**CiteCurriculum KR** · 2022 개정 교육과정 성취기준 MCP

대상: 초·중·고 교사, 교육 연구자, 에듀테크 개발자, AI 에이전트 사용자  
전제: MCP 클라이언트가 설치되어 있고 `curriculum-kr` 서버가 연결되어 있음  
...
| 클라이언트 | 팁 |
|------------|-----|
| Claude Desktop / Code | 프롬프트에 “반드시 curriculum 도구 먼저”를 시스템·프로젝트 규칙에 고정 |
| Cursor | 리포 규칙에 cite-only 워크플로 명시 |
| Codex CLI | MCP 서버 등록 후 동일 도구명 사용 |
